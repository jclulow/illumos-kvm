...
svm_vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	/*
	 * The VMRUN instruction handles the preservation of some host state;
	 * this data is stored in the per-CPU page configured via
	 * MSR_VM_HSAVE_PA, and restored on #VMEXIT.  The list of preserved
	 * registers includes at least:
	 *
	 *	ES, CS, SS, DS, GDTR, IDTR, EFER, CR0, CR3, CR4, RFLAGS, RIP,
	 *	RSP, and RAX.
	 *
	 * (from AMD Secure Virtual Machine Architecture Reference Manual)
	 *
	 * In addition, we preserve the per-CPU values of FS, GS, GS_BASE, and
	 * LDTR.  The assembly perilogue surrounding the VMLOAD-VMRUN-VMSAVE
	 * sequence also saves and restores the rest of the general purpose
	 * registers.
	 */
...
kvm_svm_init(void)
{
	...
	/*
	 * NB: VMRUN requires VMCBs (which are 4KB) to be aligned on 4KB
	 * boundaries
	 */
